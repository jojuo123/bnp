...
def retokenize(
    ...
    **kwargs
    ...
    if tokenizer.is_fast:
        ...
        space_after = [True for _ in range(len(words))]
        s = "".join([w + (" " if sp else "") for w, sp in zip(words, space_after)])
        ...
        tokenized_tmp = tokenizer(
            words,
            return_attention_mask=True,
            return_offsets_mapping=False,
            return_tensors=None,
        )
        token_list = tokenized_tmp['input_ids']
        input_ids = tokenized['input_ids']
        idx_s = 0

        # with open('test3.txt', mode='w') as f:
        #     print(space_after, file=f)
        #     print(s, file=f)
        #     print(words, file=f)
        #     print(token_list, file=f)
        #     print(input_ids, file=f)

        words_from_tokens = [-100] * len(words)
        for idx, token in enumerate(token_list):
            t = token[1]
            while input_ids[idx_s] != t:
                idx_s += 1
    
            words_from_tokens[idx] = idx_s + len(token) - 3
